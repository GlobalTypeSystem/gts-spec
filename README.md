...
**Cross-dialect `$ref`.** A `$ref` composes the referenced schema into the referrer's own validation, which a validator evaluates under a single dialect; JSON Schema does not define how to compose subschemas that declare different dialects. A GTS Type Schema and every schema reachable from it through `gts://` `$ref` (directly or transitively) MUST therefore declare the same dialect, and an implementation MUST reject a cross-dialect `$ref` rather than report a false validation verdict. This constraint applies to `$ref` composition only: because derivation is established by the chained `$id` alone (ADR-0001), a derived Type Schema that re-declares its parent's fields **without** a `$ref` composes nothing and remains valid whatever dialect it and its ancestors declare.
