...
The intent of `x-gts-ref` is to let a GTS storage/registry validate referenced entities. The field value MUST resolve to at least one registered GTS type/instance, uniformly for concrete references and every wildcard (including `gts.*`, where a `~`-terminated value matches the exact type and any descendant). Every resolved entity MUST also be valid under the transitive validation rule in §9.2; registration alone is insufficient.
...
- **Validation**
  ...
  - **Reference resolution of trait values** (`x-gts-ref`): when a trait property in the effective trait-schema is annotated with `x-gts-ref` (§9.6), the corresponding value in the *materialized* effective traits object MUST satisfy that reference. This applies to the value in force at validation time regardless of which layer supplied it — a value inherited from an ancestor's `x-gts-traits` (or from a trait-schema `default`) is resolved against the registry exactly as if the validated type had declared it directly. Per §9.6, the field value MUST be a syntactically valid GTS identifier that matches the declared constraint (concrete identifier or wildcard pattern). At least one registered, valid GTS type/instance MUST match the referenced value, applied uniformly to concrete identifiers and to every wildcard pattern, including `gts.*`; each match is validated transitively under §9.2. **Unlike the completeness check above, reference resolution is NOT skipped for `x-gts-abstract` types**: an abstract type is still validated for reference integrity — a concrete `x-gts-ref` it declares must name a registered constraint type, and any trait value it itself supplies must resolve — even though a descendant may later override that value. This check runs on `/validate-type-schema` and `/validate-entity`, and at registration when validation is enabled (`?validate=true`).
