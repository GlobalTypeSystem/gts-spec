...
- Treating `x-gts-ref` like JSON Schema string constraints:
  ...
  - Under the suggested policy, apply registry lookup only for `presence` and `full`. Keep constraint lookup separate from value lookup: concrete constraints use their exact identifier, wildcard constraints search for at least one matching registry identifier, and field or trait values use their exact identifier. Under `full`, recursively validate the required concrete targets and accept a wildcard constraint when at least one matching entity is valid.
