...
Each derived type extends the base VM with platform-specific properties:

- **`gts.x.infra.vm.base.v1~vmware.infra.vm.esxi.v1~.tsp`**
  - VMWare ESXi virtual machine
  - Adds: `vmware` object with MoRef ID, host, datacenter, VMWare Tools status, hardware version, vMotion settings

- **`gts.x.infra.vm.base.v1~nutanix.infra.vm.ahv.v1~.tsp`**
  - Nutanix AHV (Acropolis Hypervisor) virtual machine
  - Adds: `nutanix` object with cluster UUIDs, protection domain, categories, storage container, HA priority

- **`gts.x.infra.vm.base.v1~virtuozzo.infra.vm.vz.v1~.tsp`**
  - Virtuozzo virtual machine
  - Adds: `virtuozzo` object with VEID, environment type (VM/container), node info, storage cluster, HA settings

## Instance Examples (JSON)

The `instances/` directory contains concrete VM instance examples:

- **`web-server-01.json`** - VMWare ESXi web server (4 CPU, 8GB RAM, Production)
- **`db-server-01.json`** - Nutanix AHV database server (8 CPU, 32GB RAM, High Priority)
- **`app-server-01.json`** - Virtuozzo application server (2 CPU, 4GB RAM, Staging)
...
```
gts.x.infra.vm.base.v1~<vendor>.infra.vm.<platform>.v1~
```

Examples:
- `gts.x.infra.vm.base.v1~vmware.infra.vm.esxi.v1~`
- `gts.x.infra.vm.base.v1~nutanix.infra.vm.ahv.v1~`
- `gts.x.infra.vm.base.v1~virtuozzo.infra.vm.vz.v1~`
...
2. **Hybrid database storage**: Store common VM properties in indexed columns, platform-specific data in JSONB
3. **Type-safe APIs**: Validate VM data against registered schemas before storage
4. **Access control**: Use GTS patterns to grant permissions (e.g., `gts.x.infra.vm.base.v1~vmware.*`)
...
tsp compile gts.x.infra.vm.base.v1~.tsp --emit @typespec/json-schema
```
...
ajv validate -s schemas/generated/gts.x.infra.vm.base.v1~vmware.infra.vm.esxi.v1~.schema.json \
    -d instances/gts.x.infra.vm.base.v1~vmware.infra.vm.esxi.v1~web-server-01.json
```
